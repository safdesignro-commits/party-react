...
export function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass-scrolled' : 'glass'}`}>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-elevated max-h-[calc(100vh-4rem)] overflow-y-auto">
          <div className="px-4 py-4 space-y-1">
            {/* Servicii — collapsible */}
            <button
              onClick={() => setMobileSection(mobileSection === 'services' ? null : 'services')}
              className="flex items-center justify-between w-full px-3 py-2.5 rounded-xl text-sm font-semibold text-pp-heading hover:bg-gray-50"
            >
              Servicii
              <svg className={`w-4 h-4 transition-transform ${mobileSection === 'services' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </button>
            {mobileSection === 'services' && (
              <div className="pl-3 space-y-0.5">
                {services.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="flex items-center justify-between px-3 py-2 rounded-xl text-sm text-pp-body hover:bg-pp-magenta-light">
                    <span>{item.label}</span>
            ...
            {/* Evenimente — collapsible */}
            <button
              onClick={() => setMobileSection(mobileSection === 'events' ? null : 'events')}
              className="flex items-center justify-between w-full px-3 py-2.5 rounded-xl text-sm font-semibold text-pp-heading hover:bg-gray-50"
            >
              Evenimente
              <svg className={`w-4 h-4 transition-transform ${mobileSection === 'events' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </button>
            {mobileSection === 'events' && (
              <div className="pl-3 space-y-0.5">
                {events.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="block px-3 py-2 rounded-xl text-sm text-pp-body hover:bg-pp-magenta-light">
                    {item.label}
                  </Link>
                ))}
              </div>
            ...
            <Link href="/preturi-inchiriere-cabina-foto/" onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 rounded-xl text-sm font-semibold text-pp-heading hover:bg-gray-50">Prețuri</Link>
            <Link href="/contact-cabina-fotografica-booth/" onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 rounded-xl text-sm font-semibold text-pp-heading hover:bg-gray-50">Contact</Link>

            <div className="border-t border-gray-100 my-2" />
            <div className="flex gap-2 pt-2">
              <a href="tel:0745800810" className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-pp-teal-light text-pp-teal-dark font-semibold text-sm">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                0745 800 810
              </a>
              <Link href="/contact-cabina-fotografica-booth/" onClick={() => setMobileOpen(false)} className="flex-1 text-center btn-gold py-2.5 rounded-xl font-bold text-sm text-pp-heading">
                Rezervă Acum
              </Link>
            </div>
          </div>
